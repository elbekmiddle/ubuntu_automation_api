...
async function tryRefresh() {
  const { apiUrl, refreshToken } = config.load();
...
async function request(pathname, opts = {}, _retried = false) {
  ...
  let res;
  try {
    res = await fetch(`${apiUrl}${pathname}`, { ...opts, headers });
  } catch (err) {
    const e = new Error(`API'ga ulanib bo'lmadi: ${apiUrl} (${err.message})`);
    e.cause = err;
  ...
  if (res.status === 401 && !_retried && !pathname.startsWith('/auth/')) {
    const refreshed = await tryRefresh();
    if (refreshed) return request(pathname, opts, true);
  }

  if (!res.ok) {
    const message = data?.message || `${res.status} ${res.statusText}`;
    const err = new Error(Array.isArray(message) ? message.join(', ') : message);
    if (res.status === 401) err.exitCode = EXIT.AUTH_ERROR;
    else if (res.status === 403) err.exitCode = EXIT.PERMISSION_DENIED;
    else err.exitCode = EXIT.GENERAL_ERROR;
...
module.exports = {
  ...
  jobs: {
    ...
    create: (templateSlug, action, args = {}) =>
      request('/jobs', { method: 'POST', body: JSON.stringify({ templateSlug, action, args }) }),
  },
  schedules: {
  ...
  auditLogs: {
    list: (limit = 20) => request(`/audit-logs?limit=${limit}`),
  },
  system: {
