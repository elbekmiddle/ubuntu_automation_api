...
function unitNameFor(appId: string): string {
 ...
 */
export async function installAutoStart(appId: string): Promise<AutoStartResult> {
...
`;
    ...
    try {
        ...
        return {
            ...
            unitName,
            unitPath,
            message: `Birlik fayl yozildi (${unitPath}), lekin "systemctl" ishga tushirilmadi: ${
                err instanceof Error ? err.message : String(err)
            }. Qo'lda: systemctl --user enable --now ${unitName}`,
        };
    }
...
export async function uninstallAutoStart(appId: string): Promise<void> {
    const unitName = unitNameFor(appId);
...
export function isAutoStartInstalled(appId: string): boolean {
