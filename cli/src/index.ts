...
function buildProgram(): Command {
    ...
    app.command('stop <id>').description('Stop the background agent (and auto-start service) for a device')
        .action(async (id: string) => appStopCommand(id));
    ...
    template.command('public [search]').description('Search public/community templates')
        .action(async (search?: string) => templatesPublicCommand(search));
...
function rewriteShorthandArgs(argv: string[]): string[] {
...
function printUnknownShorthand(flag: string): void {
