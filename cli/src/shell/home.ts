...
type ExitSignal = 'exit';

/** Login qilinmagan holatdagi kichik menyu (docs: "You're not logged in"). */
async function loggedOutMenu(): Promise<'continue' | ExitSignal> {
    console.clear();
    printBanner();
...
async function renderHomeHeader(): Promise<void> {
    console.clear();
    printBanner();
...
async function homeMenu(): Promise<'continue' | ExitSignal> {
    ...
    switch (choice) {
        ...
        case 'account': {
            ...
            if (loggedOut) return 'continue'; // keyingi loop loggedOutMenu'ga tushadi
            break;
        }
        case 'help':
            printHelp();
