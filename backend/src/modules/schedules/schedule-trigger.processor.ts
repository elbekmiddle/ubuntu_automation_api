...
export class ScheduleTriggerProcessor extends WorkerHost {
  ...
  async process(bullJob: BullJob<{ scheduleId: string }>): Promise<void> {
    ...
    if (!schedule) {
      this.logger.warn(
        `Schedule ${scheduleId} no longer exists, skipping trigger`,
      );
      return;
    }
    if (!schedule.enabled) {
    ...
    this.logger.log(
      `Schedule ${scheduleId} fired -> ${schedule.slug}/${schedule.action}`,
    );
    ...
    await this.db.query(`UPDATE jobs SET schedule_id = $2 WHERE id = $1`, [
      job.id,
      scheduleId,
    ]);
  }
