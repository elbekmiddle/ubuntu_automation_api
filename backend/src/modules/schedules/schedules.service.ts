  ...
  async create(
    ...
    if (!CRON_RE.test(cron)) {
      throw new BadRequestException(
        'Cron ifodasi noto\'g\'ri — "minut soat kun oy hafta" formatida bo\'lishi kerak (masalan "0 3 * * *")',
      );
    }
    ...
    if (!template.actions.includes(action)) {
      throw new BadRequestException(
        `Action "${action}" template "${templateSlug}"da mavjud emas`,
      );
    }
    ...
    this.logger.log(
      `Created schedule ${schedule.id} for ${templateSlug}/${action} (${cron})`,
    );
    return schedule;
  }
