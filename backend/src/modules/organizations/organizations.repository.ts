...
export class OrganizationsRepository {
  ...
  async findAllForUser(
    userId: string,
  ): Promise<(OrganizationRow & { role: OrgRole; member_count: number })[]> {
    const { rows } = await this.db.query<
      OrganizationRow & { role: OrgRole; member_count: number }
    >(
      `SELECT o.*, m.role,
  ...
  async removeMember(memberId: string): Promise<void> {
    await this.db.query(`DELETE FROM organization_members WHERE id = $1`, [
      memberId,
    ]);
  }
