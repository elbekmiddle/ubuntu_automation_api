...
function withComputedStatus<
  T extends { status: string; last_seen_at: Date | null },
>(app: T) {
  const isRecentlyAlive =
    app.last_seen_at != null &&
    Date.now() - new Date(app.last_seen_at).getTime() < STALE_AFTER_MS;
  return {
    ...app,
    status:
      app.status === 'online' && !isRecentlyAlive ? 'offline' : app.status,
  };
}
...
export class AppsService {
  ...
  async create(
    ...
    if (machineId) {
      const existing = await this.repo.findByUserAndMachineId(
        userId,
        machineId,
      );
      if (existing) {
        const app = await this.repo.rotateToken(
          existing.id,
        ...
        this.logger.log(
          `Reconnected existing app "${app.name}" (machine ${machineId}) for user ${userId}`,
        );
  ...
  async setTags(userId: string, id: string, tags: string[]) {
    const app = await this.repo.setTags(userId, id, tags);
    if (!app) {
      throw new NotFoundException({
        code: APP_ERROR_CODES.NOT_FOUND,
        message: `${APP_ERRORS[APP_ERROR_CODES.NOT_FOUND]}: "${id}"`,
  ...
  /** Target selector — fleet automation shu bilan qaysi device'larga tegishli ekanini aniqlaydi. */
  async selectForUser(
    userId: string,
    selector: { platform?: string[]; tags?: string[]; online?: boolean },
  ) {
    const apps = await this.repo.findMatchingForUser(userId, selector);
    return apps.map(withComputedStatus);
  }
