...
const SCRIPT_TIMEOUT_MS = Number(
  process.env.SCRIPT_TIMEOUT_MS ?? 5 * 60 * 1000,
); // default 5 daqiqa
  ...
  async process(bullJob: BullJob<JobQueuePayload>): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutTimer = setTimeout(() => {
        this.logger.warn(
          `Job ${jobId} timed out after ${SCRIPT_TIMEOUT_MS}ms, killing`,
        );
        child.kill('SIGKILL');
      }, SCRIPT_TIMEOUT_MS);

      this.jobsRepo
        .markRunning(jobId, child.pid!)
        .catch((e) => this.logger.error(e));

      const truncate = (s: string) =>
        s.length > MAX_LOG_CHUNK
      ...
      child.stdout.on('data', (data: Buffer) => {
        ...
        this.jobsRepo
          .appendLog(jobId, 'stdout', chunk)
          .catch((e) => this.logger.error(e));
        this.pubsub.publish(`job:${jobId}:log`, {
          stream: 'stdout',
          chunk,
          ts: Date.now(),
        });
      ...
      child.on('close', (code) => {
        ...
        this.jobsRepo
          ...
          .then(() => {
            this.pubsub.publish(`job:${jobId}:status`, {
              status,
              exitCode: code,
            });
            resolve();
      ...
      child.on('error', (err) => {
        clearTimeout(timeoutTimer);
        this.jobsRepo
          .markFinished(jobId, 'failed', null)
          .finally(() => reject(err));
      });
    });
  }
